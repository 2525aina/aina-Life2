...
export default function PetChatPage() {
  ...
  const handleSendMessage = async () => {
    ...
    if (!userProfile?.nickname) {
      toast.error("プロフィール名が設定されていません。設定ページで設定してください。");
      return;
    }

    await sendMessage(newMessage, userProfile.nickname, userProfile.profileImageUrl);
    setNewMessage("");
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 ? (
          <div className="text-center text-gray-500 mt-8">まだメッセージはありません。</div>
        ) : (
          messages.map((msg) => (
            <div
              key={msg.id}
            ...
            >
              ...
              >
                <div className="text-sm font-semibold">
                  {msg.senderId === user?.uid ? "あなた" : msg.senderName}
                </div>
